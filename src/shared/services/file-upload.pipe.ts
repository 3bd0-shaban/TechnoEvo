...
import { ensureDir } from 'fs-extra'; // Import fs-extra for convenient directory creation
...
export class FileUploadPipe implements PipeTransform {
  ...
  constructor(private readonly configService: ConfigService<ConfigKeyPaths>) {
    const uploadPath = this.configService.get<IMiscConfig>('misc')?.upload_path;
    this.uploadPath = uploadPath || '/var/uploads/technoevo';
  }
  ...
  async transform(file: any, metadata: ArgumentMetadata): Promise<any> {
    ...
    await ensureDir(this.uploadPath); // Ensure the directory exists
